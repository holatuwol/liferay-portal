 ...
 * @author Ivica Cardic
 */
public class UpgradedPDFProcessorImpl
	...
	private void _generateImagesPB(FileVersion fileVersion, File file)
		...
		try (PDDocument pdDocument = PDDocument.load(file)) {
			if (pdDocument.isEncrypted()) {
				PDDocument decryptedPdDocument = _decryptDocument(file);

				if (decryptedPdDocument == null) {
					_log.error(
				...
				decryptedPdDocument.save(decryptedFile);

				previewFilesCount = decryptedPdDocument.getNumberOfPages();
			}
			else {
		...
		if (PropsValues.DL_FILE_ENTRY_PREVIEW_FORK_PROCESS_ENABLED) {
			...
			String classpath = ClassPathUtil.buildClassPath(
				Bundle.class, LiferayPDFBoxProcessCallable.class);
			...
			UpgradedLiferayPDFBoxConverter liferayConverter =
				new UpgradedLiferayPDFBoxConverter(
	...
	private PDDocument _decryptDocument(File file) {
		...
		for (String decryptPassword : decryptPasswords) {
			try {
				PDDocument pdDocument = PDDocument.load(file, decryptPassword);
	...
	private static final Log _log = LogFactoryUtil.getLog(
		UpgradedPDFProcessorImpl.class);
