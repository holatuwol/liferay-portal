...
public class SassFile extends TimedSassFragment {
	...
	public void writeCacheFiles() throws Exception {
		...
		String rtlFileName = SassToCssBuilder.getRtlCustomFileName(
			ltrFileName);
	...
	protected String doGetLtrContent() throws Exception {
		...
		for (SassFragment fragment : _fragments) {
			sb.append(fragment.getLtrContent());
		}

		String baseURL = _BASE_URL.concat(_baseDir);

		return AggregateUtil.updateRelativeURLs(sb.toString(), baseURL);
	}
	...
	protected String doGetRtlContent() throws Exception {
		...
		for (SassFragment fragment : _fragments) {
			sb.append(fragment.getRtlContent());
		}
